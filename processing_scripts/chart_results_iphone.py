...
# Read total.csv
total_df = pd.read_csv('iphone_total_decay.csv', header=None)
total_values = total_df.iloc[:, 1].values  # Assuming the first column is the index

# Read deviation.csv
deviation_values = []
with open('iphone_deviation_decay.csv', 'r') as file:
    for line in file:
...
for p in percent_values:
    crossings = np.where(np.isclose(norm_total_values, p, atol=0.1))[0]  # Find closest x values
